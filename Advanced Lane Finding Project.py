...
cv2.imwrite('./output_images/test_undist3.jpg',dst)
...
cv2.imwrite('./output_images/undistorted_straightlines1.jpg',exampleImg_undistort)
# Visualize undistortion
f, (ax1, ax2) = plt.subplots(1, 2, figsize=(20,10))
f.subplots_adjust(hspace = .2, wspace=.05)
...
# 
# #### Note:
# this is not very sophistocated or robust, but could possibly be made so... On the other hand, if we can assume the road is always relatively flat and the camera position is fixed, a fixed perspective transform may be appropriate.
...
exampleImg_unwarp, M, Minv = unwarp(exampleImg_undistort, src, dst) # M for warping and Minv for unwarping
cv2.imwrite('./output_images/unwarped_straightlines1.jpg',exampleImg_unwarp)
# Visualize unwarp
f, (ax1, ax2) = plt.subplots(1, 2, figsize=(20,10))
f.subplots_adjust(hspace = .2, wspace=.05)
...
def abs_sobel_thresh(img, orient='x', thresh_min=25, thresh_max=255):
    ...
    #gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)  # To grayscale
    gray = (cv2.cvtColor(img, cv2.COLOR_RGB2Lab))[:,:,0] # To LAB L channel
...
print('Sliding window def is done.')
...
video_output1 = './Output_Videos/project_video_output_new.mp4'
...
get_ipython().run_line_magic('time', 'processed_video.write_videofile(video_output2, audio=False)')
...
get_ipython().run_line_magic('time', 'processed_video.write_videofile(video_output3, audio=False)')
